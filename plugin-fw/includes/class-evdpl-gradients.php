...
/**
 * EVDPL Plugin Gradients Class.
 *
 * @class   EVDPL_Gradients
 * @package EVDPL\PluginFramework\Classes
 */
defined('ABSPATH') || exit; // Exit if accessed directly.

if (!class_exists('EVDPL_Gradients')) {

    /**
     * EVDPL_Gradients class.
     *
     */
    class EVDPL_Gradients {
        ...
        public function add_color_gradient($color, $position) {
            $the_color['color'] = esc_attr($color);
            $the_color['position'] = esc_attr($position);
        ...
        public function gradient_darker($role, $color, $direction, $factor = 30) {
            ...
            $colors = array(
                ...
                array(
                    'color' => esc_attr($this->hex_darker($color, $factor)),
                    'position' => 100,
                ),
            );
        ...
        public function gradient_lighter($role, $color, $direction, $factor = 30) {
            ...
            $colors = array(
                ...
                array(
                    'color' => esc_attr($this->hex_lighter($color, $factor)),
                    'position' => 100,
                ),
            );
        ...
        private function make_modern_gradient($colors, $direction, $browser) {
            ...
            switch ($direction) {
                case 'vertical':
                    $css .= esc_attr($browser) . 'linear-gradient(top,';
                    break;
                case 'horizontal':
                    $css .= esc_attr($browser) . 'linear-gradient(left,';
                    break;
                case 'diagonal-bottom':
                    $css .= esc_attr($browser) . 'linear-gradient(-45deg,';
                    break;
                case 'diagonal-top':
                    $css .= esc_attr($browser) . 'linear-gradient(45deg,';
                    break;
                case 'radial':
                    $css .= esc_attr($browser) . 'radial-gradient(center, ellipse cover,';
                    break;
            }

            foreach ($colors as $stop) {
                $css .= esc_attr($stop['color']) . ' ' . esc_attr($stop['position']) . '%, ';
            }
        ...
        private function make_chr_saf4_gradient($colors, $direction) {
            ...
            foreach ($colors as $stop) {
                $css .= 'color-stop(' . esc_attr($stop['position']) . '%, ' . esc_attr($stop['color']) . '), ';
            }
        ...
        public function get_model($class) {
            return class_exists('EVDPL_Registry') ? EVDPL_Registry::get_instance()->$class : false;
        }
        ...
        public function __get($prop) {
            $value = null;
            if ('request' === $prop) {
                if (class_exists('EVDPL_Request') && (!isset($this->request) || !$this->request instanceof EVDPL_Request )) {
                    $value = EVDPL_Registry::get_instance()->request;
                    $this->request = $value;
                }
            }
