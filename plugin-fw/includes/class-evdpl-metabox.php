...
if (!class_exists('EVDPL_Metabox')) {
    ...
    class EVDPL_Metabox {
        ...
        public function init($options = array()) {
            ...
            add_filter('evdpl_icons_screen_ids', array($this, 'add_screen_ids_for_icons'));

            add_action('wp_ajax_evdpl_plugin_fw_save_toggle_element_metabox', array($this, 'save_toggle_element'));
        }
        ...
        public function enqueue() {
            ...
            $enqueue = apply_filters('evdpl_plugin_fw_metabox_enqueue_styles_and_scripts', $enqueue, $this);
            ...
            if ($enqueue) {
                ...
                wp_enqueue_style('evdpl-plugin-metaboxes');
                wp_enqueue_style('jquery-ui-style');

                wp_enqueue_script('evdpl-metabox');
        ...
        public function get_option_metabox_name($field_id, $private = true) {
            $db_name = apply_filters('evdpl_metaboxes_option_main_name', 'evdpl_metaboxes');
            $return = $db_name . '[';
        ...
        public function show($post, $meta_box) {
            ...
            $args = array(
                'tabs' => $this->tabs,
                'class' => isset($this->options['class']) ? esc_attr($this->options['class']) : '',
            );

            if (isset($meta_box, $meta_box['id'])) {
                do_action("evdpl_plugin_fw_metabox_before_render_{$meta_box['id']}", $post, $meta_box);
            }

            evdpl_plugin_get_template(EVDPL_CORE_PLUGIN_PATH, 'metaboxes/tab.php', $args);
        }
        ...
        public function save_postdata($post_id) {
            if (!isset($_POST['evdpl_metaboxes_nonce']) || !wp_verify_nonce(wp_unslash($_POST['evdpl_metaboxes_nonce']), 'metaboxes-fields-nonce')) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
                return $post_id;
            }

            $allow_ajax = isset($_REQUEST['evdpl_metabox_allow_ajax_saving']) && sanitize_key(wp_unslash($_REQUEST['evdpl_metabox_allow_ajax_saving'])) === $this->id;
            ...
            if (isset($_POST['evdpl_metaboxes'])) {
                $evdpl_metabox_data = wp_unslash($_POST['evdpl_metaboxes']); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized

                if (is_array($evdpl_metabox_data)) {
                    foreach ($evdpl_metabox_data as $field_name => $field_value) {
        ...
        public function sanitize_and_save_fields($post_id) {
            ...
            $allow_ajax = isset($_REQUEST['evdpl_metabox_allow_ajax_saving']) && sanitize_key(wp_unslash($_REQUEST['evdpl_metabox_allow_ajax_saving'])) === $this->id;
            $ajax_partial_saving_tab = isset($_REQUEST['evdpl_metabox_allow_ajax_partial_saving_tab']) ? sanitize_key(wp_unslash($_REQUEST['evdpl_metabox_allow_ajax_partial_saving_tab'])) : false;
        ...
        public function sanitize_and_save_field($field, $post_id) {
            ...
            $meta_box_data = isset($_POST['evdpl_metaboxes']) ? wp_unslash($_POST['evdpl_metaboxes']) : array(); #phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
            ...
            if (isset($meta_box_data[$field['id']])) {
                if (in_array($field['type'], array('onoff', 'checkbox'), true)) {
                        ...
                        if ($elements_value) {
                            ...
                            foreach ($field['elements'] as $element) {
                                foreach ($elements_value as $key => $element_value) {
                                    ...
                                    if (!empty($element['evdpl-sanitize-callback']) && is_callable($element['evdpl-sanitize-callback'])) {
                                        $elements_value[$key][$element['id']] = call_user_func($element['evdpl-sanitize-callback'], $elements_value[$key][$element['id']]);
                                    }
                                }
                            }
                        }
                    ...
                    if (!empty($field['evdpl-sanitize-callback']) && is_callable($field['evdpl-sanitize-callback'])) {
                        $value = call_user_func($field['evdpl-sanitize-callback'], $value);
                    }
        ...
        public function save_toggle_element() {
            ...
            if (!isset($_REQUEST['evdpl_metaboxes_nonce']) || !wp_verify_nonce(wp_unslash($_REQUEST['evdpl_metaboxes_nonce']), 'metaboxes-fields-nonce')) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
                return;
            }

            $post_id = isset($_REQUEST['post_ID']) ? absint(sanitize_text_field($_REQUEST['post_ID'])) : false;
            if (!$post_id) {
                return;
            }

            if (isset($_REQUEST['evdpl_metaboxes'], $_REQUEST['toggle_id'], $_REQUEST['metabox_tab'], $_REQUEST['evdpl_metaboxes'][$_REQUEST['toggle_id']])) {
                $meta_box_data = isset($_REQUEST['evdpl_metaboxes']) ? wp_unslash($_REQUEST['evdpl_metaboxes']) : array(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
...
if (!function_exists('evdpl_metabox')) {
     ...
     * @return EVDPL_Metabox
     */
    function evdpl_metabox($id) {
        return EVDPL_Metabox::instance($id);
    }
