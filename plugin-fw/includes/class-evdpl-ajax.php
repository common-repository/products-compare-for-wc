    ...
    class EVDPL_Ajax {
        ...
        private function __construct() {
            ...
            foreach ($ajax_actions as $ajax_action) {
                add_action('wp_ajax_evdpl_plugin_fw_' . $ajax_action, array($this, $ajax_action));
                add_action('wp_ajax_nopriv_evdpl_plugin_fw_' . $ajax_action, array($this, $ajax_action));
            }
        }
        ...
        public function json_search_posts($request = array()) {
            ...
            if (!empty($posts)) {
                foreach ($posts as $post_id) {
                    ...
                    $found_posts[$post_id] = apply_filters('evdpl_plugin_fw_json_search_found_post_title', rawurldecode($title), $post_id, $request);
                }
            }

            $found_posts = apply_filters('evdpl_plugin_fw_json_search_found_posts', $found_posts, $request);
        ...
        public function json_search_products() {
            ...
            $request = apply_filters('evdpl_plugin_fw_json_search_products_request', $request);
        ...
        public function json_search_orders() {
            ...
            $term = isset($_REQUEST['term']) ? sanitize_text_field(intval($_REQUEST['term'])) : false;
            ...
            $found_orders = array();
            $term = apply_filters('evdpl_plugin_fw_json_search_order_number', $term);
            $search = '%' . intval($term) . '%';
            ...
            if ($query_orders) {
                foreach ($query_orders as $item) {
                    $order_number = apply_filters('evdpl_plugin_fw_order_number', '#' . $item->ID, $item->ID);
        ...
        public function json_search_terms() {
            ...
            $args = apply_filters(
                    'evdpl_plugin_fw_json_search_terms_default_args',
                    array(
                        'taxonomy' => 'category',
                        'hide_empty' => false,
                        'order' => 'ASC',
            ...
            $args = apply_filters('evdpl_plugin_fw_json_search_terms_args', $args);
