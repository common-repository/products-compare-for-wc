 ...
 */
import { ajaxFetch, generateShortcode, evdpl_icon } from '../../common';
import md5                                         from 'md5';
import './style.scss';

const BEFORE_DO_SHORTCODE_ACTION  = 'evdpl_plugin_fw_gutenberg_before_do_shortcode';
const SUCCESS_DO_SHORTCODE_ACTION = 'evdpl_plugin_fw_gutenberg_success_do_shortcode';
const AFTER_DO_SHORTCODE_ACTION   = 'evdpl_plugin_fw_gutenberg_after_do_shortcode';
...
export class Shortcode extends Component {
	...
	updateShortcode() {
		...
		if ( blockArgs.do_shortcode ) {
			...
			this.ajaxTimeout = setTimeout( () => {
				const ajaxData = {
					action   : 'evdpl_plugin_fw_gutenberg_do_shortcode',
					shortcode: shortcode
				};
	...
	render() {
		...
		const mainClass = 'block-editor-evdpl-plugin-fw-shortcode-block';
		...
		wrapperClasses.push( `evdpl_block_${shortcodeHash}` );
		...
		return (
			<>
				<div className={wrapperClasses.join( ' ' )}>
					...
					{showTitle &&
					 <div className={`${mainClass}__title components-placeholder__label`}>{evdpl_icon}{title}</div>
					}
					{showMessage &&
