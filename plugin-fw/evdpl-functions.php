...
if (!function_exists('evdpl_plugin_locate_template')) {
     ...
     * @since 1.0.0
     */
    function evdpl_plugin_locate_template($plugin_basename, $path, $var = null) {
...
if (!function_exists('evdpl_plugin_get_template')) {
     ...
     * @since 1.0.0
     */
    function evdpl_plugin_get_template($plugin_basename, $path, $var = null, $return = false) {

        $located = evdpl_plugin_locate_template($plugin_basename, $path, $var);
...
if (!function_exists('evdpl_plugin_content')) {
     ...
     * @since      1.0.0
     */
    function evdpl_plugin_content($what = 'content', $limit = 25, $more_text = '', $split = '[...]', $in_paragraph = true) {
        if ('content' === $what) {
        ...
        if (!empty($more_text) && !isset($more_link)) {
            $more_link = strpos($more_text, '<a class="btn"') ? $more_text : '<a class="read-more' . apply_filters('evdpl_simple_read_more_classes', ' ') . '" href="' . get_permalink() . '">' . $more_text . '</a>';
            $split = '';
...
if (!function_exists('evdpl_plugin_string')) {
     ...
     * @deprecated 3.5
     */
    function evdpl_plugin_string($before = '', $string = '', $after = '', $echo = true) {
        $html = '';
...
if (!function_exists('evdpl_plugin_decode_title')) {
     ...
     * @since      1.0.0
     */
    function evdpl_plugin_decode_title($title) {
        $replaces = apply_filters('evdpl_title_special_characters', array());
...
if (!function_exists('evdpl_plugin_get_attachment_id')) {
     ...
     * @since 1.0.0
     */
    function evdpl_plugin_get_attachment_id($url) {
...
if (!function_exists('evdpl_enqueue_script')) {

    /**
     * Enqueues script.
     * Registers the script if src provided (does NOT overwrite) and enqueues.
     * IMPORTANT: used only in themes, since it needs the EVDPL_Asset class.
     *
     ...
     *
     */
    function evdpl_enqueue_script($handle, $src, $deps = array(), $ver = false, $in_footer = true) {
        if (function_exists('EVDPL_Asset') && !is_admin()) {
            $enqueue = true;
            EVDPL_Asset()->set('script', $handle, compact('src', 'deps', 'ver', 'in_footer', 'enqueue'));
        } else {
...
if (!function_exists('evdpl_enqueue_style')) {
     ...
     *
     */
    function evdpl_enqueue_style($handle, $src, $deps = array(), $ver = false, $media = 'all') {
        if (function_exists('EVDPL_Asset')) {
            $enqueue = true;
            $who = EVDPL_Asset()->get_stylesheet_handle(get_stylesheet_uri(), 'style');
            $where = 'before';
            ...
            EVDPL_Asset()->set('style', $handle, compact('src', 'deps', 'ver', 'media', 'enqueue'), $where, $who);
        } else {
...
if (!function_exists('evdpl_get_post_meta')) {
     ...
     * @since    1.0.0
     */
    function evdpl_get_post_meta($id, $meta) {
        if (!strpos($meta, '[')) {
...
if (!function_exists('evdpl_string')) {
     ...
     * @since      1.0.0
     */
    function evdpl_string($before = '', $string = '', $after = '', $echo = true) {
        $html = '';
...
if (!function_exists('evdpl_pagination')) {
     ...
     * @since      1.0.0
     */
    function evdpl_pagination($pages = '', $range = 10) {
        ...
        if (1 !== $pages) {
            ...
            if ($paged > 2) {
                $html .= sprintf('<a class="%s" href="%s">&laquo;</a>', 'evdpl_pagination_first', get_pagenum_link(1));
            }
            if ($paged > 1) {
                $html .= sprintf('<a class="%s" href="%s">&lsaquo;</a>', 'evdpl_pagination_previous', get_pagenum_link($paged - 1));
            }
            ...
            if ($paged < $pages) {
                $html .= sprintf('<a class="%s" href="%s">&rsaquo;</a>', 'evdpl_pagination_next', esc_url(get_pagenum_link($paged + 1)));
            }
            if ($paged < $pages - 1) {
                $html .= sprintf('<a class="%s" href="%s">&raquo;</a>', 'evdpl_pagination_last', esc_url(get_pagenum_link($pages)));
            }

            $html .= "</div>\n";
        }

        echo apply_filters('evdpl_pagination_html', $html); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    }

}

if (!function_exists('evdpl_registered_sidebars')) {
     ...
     * @since      1.0.0
     */
    function evdpl_registered_sidebars() {
...
if (!function_exists('evdpl_layout_option')) {

    /**
     * Retrieve a layout option
     * IMPORTANT: used only in themes, since it needs the EVDPL_Layout_Panel class.
     *
     ...
     * @since      1.0.0
     */
    function evdpl_layout_option($key, $id = false, $type = 'post', $model = 'post_type') {
        $option = '';

        if (defined('EVDPL')) {
            $option = EVDPL_Layout_Panel()->get_option($key, $id, $type, $model);
        } else {
...
if (!function_exists('evdpl_curPageURL')) {
     ...
     * @since 1.0
     */
    function evdpl_curPageURL() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
...
if (!function_exists('evdpl_get_excluded_categories')) {
     ...
     * @since      1.0.0
     */
    function evdpl_get_excluded_categories($k = 1) {
        global $post;
        ...
        $cats = function_exists('evdpl_get_option') ? evdpl_get_option('blog-excluded-cats') : '';
...
if (!function_exists('evdpl_add_extra_theme_headers')) {
    add_filter('extra_theme_headers', 'evdpl_add_extra_theme_headers');
     ...
     * @since  1.0.0
     */
    function evdpl_add_extra_theme_headers($headers) {
...
if (!function_exists('evdpl_check_plugin_support')) {
     ...
     * @since  1.0.0
     */
    function evdpl_check_plugin_support() {
        $headers['core'] = wp_get_theme()->get('Core Framework Version');
        $headers['author'] = wp_get_theme()->get('Author');

        if (!$headers['core'] && defined('EVDPL_CORE_VERSION')) {
            $headers['core'] = EVDPL_CORE_VERSION;
        }
...
if (!function_exists('evdpl_ie_version')) {
...
if (!function_exists('evdpl_avoid_duplicate')) {
     ...
     * @since  1.0.0
     */
    function evdpl_avoid_duplicate($value, $array, $check = 'value') {
        $match = array();
        ...
        if (( 'value' === $check && !in_array($value, $array, true) ) || ( 'key' === $check && !isset($array[$value]) )) {
            ...
            return evdpl_avoid_duplicate($value . '-' . $i, $array, $check);
        }
    }

}

if (!function_exists('evdpl_title_special_characters')) {

    /**
     * The chars used in evdpl_decode_title() and evdpl_encode_title()
     * E.G.
     * string: This is [my title] with | a new line
     ...
     * @since      1.0.0
     */
    function evdpl_title_special_characters($chars) {
        return array_merge(
                $chars,
                array(
    ...
    add_filter('evdpl_title_special_characters', 'evdpl_title_special_characters');
}

if (!function_exists('evdpl_decode_title')) {
     ...
     * @since      1.0.0
     */
    function evdpl_decode_title($title) {
        $replaces = apply_filters('evdpl_title_special_characters', array());
...
if (!function_exists('evdpl_encode_title')) {
     ...
     * @since      1.0.0
     */
    function evdpl_encode_title($title) {
        $replaces = apply_filters('evdpl_title_special_characters', array());
...
if (!function_exists('evdpl_remove_chars_title')) {
     ...
     * @since      1.0.0
     */
    function evdpl_remove_chars_title($title) {
        $replaces = apply_filters('evdpl_title_special_characters', array());
...
if (!function_exists('evdpl_load_js_file')) {
     ...
     * @since  1.0.0
     */
    function evdpl_load_js_file($filename) {

        if (!( ( defined('WP_DEBUG') && WP_DEBUG ) || ( defined('SCRIPT_DEBUG') && SCRIPT_DEBUG ) || isset($_GET['evdpl_script_debug']) )) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
...
if (!function_exists('evdpl_load_css_file')) {
     ...
     * @since  1.0.0
     */
    function evdpl_load_css_file($filename) {
...
if (!function_exists('evdpl_wpml_register_string')) {
     ...
     * @since  1.0.0
     */
    function evdpl_wpml_register_string($context, $name, $value) {
        do_action('wpml_register_single_string', $context, $name, $value);
    }

}

if (!function_exists('evdpl_wpml_string_translate')) {
     ...
     * @since  1.0.0
     */
    function evdpl_wpml_string_translate($context, $name, $default_value) {
        return apply_filters('wpml_translate_single_string', $default_value, $context, $name);
    }

}

if (!function_exists('evdpl_wpml_object_id')) {
     ...
     * @since  1.0.0
     */
    function evdpl_wpml_object_id($element_id, $element_type = 'post', $return_original_if_missing = false, $language_code = null) {
...
if (!function_exists('evdpl_get_formatted_price')) {
     ...
     * @return string
     */
    function evdpl_get_formatted_price($price, $args = array()) {
        $defaults = array(
            'ex_tax_label' => false,
            'currency' => '',
        ...
        list ( $decimals, $decimal_separator, $thousand_separator, $price_format, $currency ) = evdpl_plugin_fw_extract($args, 'decimals', 'decimal_separator', 'thousand_separator', 'price_format', 'currency');
...
if (!function_exists('evdpl_get_terms')) {
...
if (!function_exists('evdpl_field_deps_data')) {
     ...
     * @return string
     */
    function evdpl_field_deps_data($field) {
        $deps_data = '';
...
if (!function_exists('evdpl_panel_field_deps_data')) {

    /**
     * Retrieve the panel field deps HTML data.
     *
     * @param array                                         $field The field.
     * @param EVDPL_Plugin_Panel|EVDPL_Plugin_Panel_WooCommerce $panel The panel object.
     *
     * @return string
     */
    function evdpl_panel_field_deps_data($field, $panel) {
        $deps_data = '';
        if (isset($field['deps']) && ( isset($field['deps']['ids']) || isset($field['deps']['id']) ) && isset($field['deps']['values'])) {
            ...
            $deps_data = evdpl_field_deps_data($field);
        }
...
if (!function_exists('evdpl_plugin_fw_get_field')) {
     ...
     * @return false|string
     */
    function evdpl_plugin_fw_get_field($field, $echo = false, $show_container = true) {
        if (empty($field['type'])) {
            return '';
        }
        ...
        $field_template = evdpl_plugin_fw_get_field_template_path($field);

        if (!isset($field['id'])) {
            static $field_number = 1;

            $field['id'] = "evdpl-plugin-fw-field__{$field_number}";
            $field_number++;
        }
        ...
        if ($field_template) {
            ...
            if ($show_container) {
                echo '<div class="evdpl-plugin-fw-field-wrapper evdpl-plugin-fw-' . esc_attr($field['type']) . '-field-wrapper">';
            }

            do_action('evdpl_plugin_fw_get_field_before', $field);
            do_action('evdpl_plugin_fw_get_field_' . $field['type'] . '_before', $field);

            include $field_template;

            do_action('evdpl_plugin_fw_get_field_after', $field);
            do_action('evdpl_plugin_fw_get_field_' . $field['type'] . '_after', $field);
...
if (!function_exists('evdpl_plugin_fw_get_field_template_path')) {
     ...
     * @return false|string
     */
    function evdpl_plugin_fw_get_field_template_path($field) {
        if (empty($field['type'])) {
            return false;
        }

        $field_template = EVDPL_CORE_PLUGIN_TEMPLATE_PATH . '/fields/' . sanitize_title($field['type']) . '.php';

        $field_template = apply_filters('evdpl_plugin_fw_get_field_template_path', $field_template, $field);
...
if (!function_exists('evdpl_plugin_fw_html_data_to_string')) {
     ...
     * @return string
     */
    function evdpl_plugin_fw_html_data_to_string($data = array(), $echo = false) {
        $html_data = '';
...
if (!function_exists('evdpl_plugin_fw_get_icon')) {
     ...
     * @return string
     */
    function evdpl_plugin_fw_get_icon($icon = '', $args = array()) {
        return EVDPL_Icons()->get_icon($icon, $args);
    }

}

if (!function_exists('evdpl_plugin_fw_is_true')) {
     ...
     * @return bool
     */
    function evdpl_plugin_fw_is_true($value) {
        return true === $value || 1 === $value || '1' === $value || 'yes' === $value || 'true' === $value;
    }

}

if (!function_exists('evdpl_plugin_fw_enqueue_enhanced_select')) {

    /**
     * Enqueue the enhanced select style and script.
     */
    function evdpl_plugin_fw_enqueue_enhanced_select() {
        wp_enqueue_script('evdpl-enhanced-select');
        $select2_style_to_enqueue = function_exists('WC') ? 'woocommerce_admin_styles' : 'evdpl-select2-no-wc';
        wp_enqueue_style($select2_style_to_enqueue);
    }

}

if (!function_exists('evdpl_add_select2_fields')) {
     ...
     */
    function evdpl_add_select2_fields($args = array()) {
        $default = array(
            'type' => 'hidden',
            'class' => '',
            'id' => '',
        ...
        $template = EVDPL_CORE_PLUGIN_TEMPLATE_PATH . '/fields/resources/' . $select2_template_name;
...
if (!function_exists('evdpl_plugin_fw_get_version')) {
     ...
     * @return string
     */
    function evdpl_plugin_fw_get_version() {
        $plugin_fw_data = get_file_data(trailingslashit(EVDPL_CORE_PLUGIN_PATH) . 'init.php', array('Version' => 'Version'));
...
if (!function_exists('evdpl_plugin_fw_is_panel')) {
     ...
     * @return bool
     */
    function evdpl_plugin_fw_is_panel() {
        $panel_screen_id = 'evdpl-plugins_page';
...
if (!function_exists('evdpl_plugin_fw_force_regenerate_plugin_update_transient')) {
     ...
     */
    function evdpl_plugin_fw_force_regenerate_plugin_update_transient() {
        delete_site_transient('update_plugins');
    }

}

if (!function_exists('evdpl_plugin_fw_is_gutenberg_enabled')) {
     ...
     * @return bool
     */
    function evdpl_plugin_fw_is_gutenberg_enabled() {
        return function_exists('EVDPL_Gutenberg');
    }

}

if (!function_exists('evdpl_plugin_fw_gutenberg_add_blocks')) {
     ...
     */
    function evdpl_plugin_fw_gutenberg_add_blocks($blocks) {
        $added = false;
        if (evdpl_plugin_fw_is_gutenberg_enabled()) {
            // Add blocks.
            $added = EVDPL_Gutenberg()->add_blocks($blocks);

            // Add blocks arguments.
            if ($added) {
                EVDPL_Gutenberg()->set_block_args($blocks);
            }
        }
...
if (!function_exists('evdpl_plugin_fw_gutenberg_get_registered_blocks')) {
     ...
     * @return array
     */
    function evdpl_plugin_fw_gutenberg_get_registered_blocks() {
        return evdpl_plugin_fw_is_gutenberg_enabled() ? EVDPL_Gutenberg()->get_registered_blocks() : array();
    }

}

if (!function_exists('evdpl_plugin_fw_gutenberg_get_to_register_blocks')) {
     ...
     * @return array
     */
    function evdpl_plugin_fw_gutenberg_get_to_register_blocks() {
        return evdpl_plugin_fw_is_gutenberg_enabled() ? EVDPL_Gutenberg()->get_to_register_blocks() : array();
    }

}

if (!function_exists('evdpl_plugin_fw_get_default_logo')) {
     ...
     */
    function evdpl_plugin_fw_get_default_logo() {
        return EVDPL_CORE_PLUGIN_URL . '/assets/images/logo.png';
    }

}

if (!function_exists('evdpl_get_wrapper_class')) {
     ...
     * @since 1.0.0
     */
    function evdpl_get_wrapper_class() {
        return 'evdpl-plugin-ui';
    }

}

if (!function_exists('evdpl_set_wrapper_class')) {
     ...
     * @return string
     */
    function evdpl_set_wrapper_class($class = '') {
        $new_class = evdpl_get_wrapper_class();
...
if (!function_exists('evdpl_get_date_formats')) {
     ...
     * @return array
     */
    function evdpl_get_date_format($js = true) {
        return evdpl_get_date_formats($js);
    }
...
if (!function_exists('evdpl_get_date_formats')) {
     ...
     * @return array
     */
    function evdpl_get_date_formats($js = true) {
        $date_formats = array(
            'F j, Y' => 'F j, Y',
            'Y-m-d' => 'Y-m-d',
            'm/d/Y' => 'm/d/Y',
        ...
        return apply_filters('evdpl_plugin_fw_date_formats', $date_formats, $js);
    }

}

if (!function_exists('evdpl_get_time_formats')) {
     ...
     * @return array
     */
    function evdpl_get_time_formats() {
        ...
        return apply_filters('evdpl_plugin_fw_time_formats', $time_formats);
    }

}


if (!function_exists('evdpl_format_toggle_title')) {
     ...
     * @return array
     */
    function evdpl_format_toggle_title($title, $values) {
...
if (!function_exists('evdpl_plugin_fw_load_update_and_licence_files')) {

    /**
     * Load premium file for license and update system
     *
     */
    function evdpl_plugin_fw_load_update_and_licence_files() {
        ...
        if (empty($plugin_upgrade_fw_data)) {
            ...
            if (file_exists($plugin_upgrade_path)) {
                $required_files = array(
                    $plugin_upgrade_path . '/lib/evdpl-plugin-upgrade.php',
                );
...
if (!function_exists('evdpl_plugin_fw_remove_duplicate_classes')) {
     ...
     * @since  1.0.0
     */
    function evdpl_plugin_fw_remove_duplicate_classes($classes) {
...
if (!function_exists('evdpl_plugin_fw_add_requirements')) {
     ...
     */
    function evdpl_plugin_fw_add_requirements($plugin_name, $requirements) {
        if (!empty($requirements)) {
            EVDPL_System_Status()->add_requirements($plugin_name, $requirements);
        }
    }

}

if (!function_exists('evdpl_plugin_fw_parse_dimensions')) {
     ...
     * @return array
     */
    function evdpl_plugin_fw_parse_dimensions($values) {
        $dimensions = array();
...
if (!function_exists('evdpl_plugin_fw_get_dimensions_by_option')) {
     ...
     * @return array|bool
     */
    function evdpl_plugin_fw_get_dimensions_by_option($option, $default = false) {
        $dimensions = get_option($option, false);

        return !!$dimensions ? evdpl_plugin_fw_parse_dimensions($dimensions) : $default;
    }

}

if (!function_exists('evdpl_plugin_fw_extract')) {
     ...
     * Usage example:
     * ```
     * list ( $type, $class, $value ) = evdpl_plugin_fw_extract( $field, 'type', 'class', 'value' );
     * ```
     *
     ...
     * @return array
     */
    function evdpl_plugin_fw_extract($array, ...$keys) {
        return array_map(
...
if (!function_exists('evdpl_plugin_fw_register_elementor_widget')) {
     ...
     *
     */
    function evdpl_plugin_fw_register_elementor_widget($widget_name, $widget_options) {
        EVDPL_Elementor::instance()->register_widget($widget_name, $widget_options);
    }

}

if (!function_exists('evdpl_plugin_fw_register_elementor_widgets')) {
     ...
     *
     */
    function evdpl_plugin_fw_register_elementor_widgets($widgets, $map_from_gutenberg = false) {
        foreach ($widgets as $widget_name => $widget_options) {
            ...
            evdpl_plugin_fw_register_elementor_widget($widget_name, $widget_options);
        }
    }

}

if (!function_exists('evdpl_plugin_fw_copy_to_clipboard')) {
     ...
     *
     */
    function evdpl_plugin_fw_copy_to_clipboard($value, $field = array()) {
        $defaults = array(
            'id' => '',
            'value' => $value,
        );
        ...
        evdpl_plugin_fw_get_field($field, true, false);
    }

}

if (!function_exists('evdpl_plugin_fw_add_utm_data')) {
     ...
     *
     */
    function evdpl_plugin_fw_add_utm_data($url, $slug, $campaign = 'plugin-version-author-uri', $source = 'wp-dashboard') {
...
if (!function_exists('evdpl_plugin_fw_include_fw_template')) {
     ...
     *
     */
    function evdpl_plugin_fw_include_fw_template($template, $args = array()) {
        $_template_path = trailingslashit(EVDPL_CORE_PLUGIN_TEMPLATE_PATH) . $template;
...
if (!function_exists('evdpl_plugin_fw_html_attributes_to_string')) {
     ...
     * @return string
     */
    function evdpl_plugin_fw_html_attributes_to_string($attributes = array(), $echo = false) {
        $html_attributes = '';
...
if (!function_exists('evdpl_plugin_fw_get_component')) {
     ...
     * @return false|string
     */
    function evdpl_plugin_fw_get_component($component, $echo = true) {
        if (!empty($component['type'])) {
            ...
            $component['html_attributes'] = evdpl_plugin_fw_html_attributes_to_string($component['attributes']);
            $component['html_data'] = evdpl_plugin_fw_html_data_to_string($component['data']);
            ...
            evdpl_plugin_fw_include_fw_template($component_template, compact('component'));
...
if (!function_exists('evdpl_plugin_fw_get_default_post_actions')) {
     ...
     * @return array
     */
    function evdpl_plugin_fw_get_default_post_actions($post, $args = array()) {
        $post = get_post($post);
        $actions = array();
        if ($post) {
            ...
            $defaults = array(
                ...
                'confirm-trash-message' => sprintf(__('Are you sure you want to move "%s" to trash?', 'evdpl-plugin-fw'), '<strong>' . $title . '</strong>'),
                // translators: %s is the title of the post object.
                'confirm-delete-message' => sprintf(__('Are you sure you want to delete "%s"?', 'evdpl-plugin-fw'), '<strong>' . $title . '</strong>') . '<br /><br />' . __('This action cannot be undone and you will be not able to recover this data.', 'evdpl-plugin-fw'),
            );
            ...
            if (is_post_type_viewable($post_type_object)) {
                if (in_array($post->post_status, array('pending', 'draft', 'future'), true)) {
                    if ($can_edit_post) {
                        ...
                        $actions['view'] = array(
                            'type' => 'action-button',
                            'title' => _x('Preview', 'Post action', 'evdpl-plugin-fw'),
                            'action' => 'view',
                            'icon' => 'eye',
                            'url' => $preview_link,
                        );
                    ...
                    $actions['view'] = array(
                        'type' => 'action-button',
                        'title' => _x('View', 'Post action', 'evdpl-plugin-fw'),
                        'action' => 'view',
                        'icon' => 'eye',
            ...
            if ($can_edit_post && 'trash' !== $post->post_status) {
                $actions['edit'] = array(
                    'type' => 'action-button',
                    'title' => _x('Edit', 'Post action', 'evdpl-plugin-fw'),
                    'action' => 'edit',
                ...
                if ($args['duplicate-url']) {
                    $actions['duplicate'] = array(
                        'type' => 'action-button',
                        'title' => _x('Duplicate', 'Post action', 'evdpl-plugin-fw'),
                        'action' => 'duplicate',
                        'icon' => 'clone',
            ...
            if (current_user_can('delete_post', $post->ID)) {
                if ('trash' === $post->post_status) {
                    $actions['untrash'] = array(
                        'type' => 'action-button',
                        'title' => _x('Restore', 'Post action', 'evdpl-plugin-fw'),
                        'action' => 'untrash',
                        'icon' => 'reply',
                    ...
                    $actions['trash'] = array(
                        'type' => 'action-button',
                        'title' => _x('Trash', 'Post action', 'evdpl-plugin-fw'),
                        'action' => 'trash',
                        'url' => get_delete_post_link($post->ID),
                    );
                    if ($args['confirm-trash-message']) {
                        $actions['trash']['confirm_data'] = array(
                            'title' => __('Confirm trash', 'evdpl-plugin-fw'),
                            'message' => $args['confirm-trash-message'],
                            'confirm-button' => _x('Yes, trash', 'Trash confirmation action', 'evdpl-plugin-fw'),
                ...
                if ('trash' === $post->post_status || !EMPTY_TRASH_DAYS) {
                    $actions['delete'] = array(
                        'type' => 'action-button',
                        'title' => _x('Delete Permanently', 'Post action', 'evdpl-plugin-fw'),
                        'action' => 'delete',
                        'icon' => 'trash',
                    ...
                    if ($args['confirm-delete-message']) {
                        $actions['delete']['confirm_data'] = array(
                            'title' => __('Confirm delete', 'evdpl-plugin-fw'),
                            'message' => $args['confirm-delete-message'],
                            'confirm-button' => _x('Yes, delete', 'Delete confirmation action', 'evdpl-plugin-fw'),
                            'confirm-button-type' => 'delete',
                        );
                    }
                }
            ...
            if ($args['more-menu']) {
                if ('trash' !== $post->post_status || true === $args['more-menu-in-trash']) {
                    $actions['more'] = array(
                        'type' => 'action-button',
                        'title' => __('Further actions', 'evdpl-plugin-fw'),
                        'action' => 'more',
                        'url' => '#',
                        'menu' => $args['more-menu'],
                    );
...
if (!function_exists('evdpl_plugin_fw_get_default_term_actions')) {
     ...
     * @return array
     */
    function evdpl_plugin_fw_get_default_term_actions($term, $args = array()) {
        ...
        if (!empty($term->term_id) && $tax) {
            ...
            if (is_taxonomy_viewable($tax)) {
                $actions['view'] = array(
                    'type' => 'action-button',
                    'title' => _x('View', 'Term action', 'evdpl-plugin-fw'),
                    'action' => 'view',
                    'icon' => 'eye',
            ...
            if (current_user_can('edit_term', $term->term_id)) {
                $actions['edit'] = array(
                    'type' => 'action-button',
                    'title' => _x('Edit', 'Term action', 'evdpl-plugin-fw'),
                    'action' => 'edit',
                    'url' => $edit_link,
                );
                ...
                if ($args['duplicate-url']) {
                    $actions['duplicate'] = array(
                        'type' => 'action-button',
                        'title' => _x('Duplicate', 'Term action', 'evdpl-plugin-fw'),
                        'action' => 'duplicate',
                        'icon' => 'clone',
                        'url' => $args['duplicate-url'],
                    );
            ...
            if (current_user_can('delete_term', $term->term_id)) {
                ...
                $actions['delete'] = array(
                    'type' => 'action-button',
                    'title' => _x('Delete', 'Term action', 'evdpl-plugin-fw'),
                    'action' => 'delete',
                    'icon' => 'trash',
                    'url' => $delete_url,
                );
...
if (!function_exists('evdpl_plugin_fw_get_action_buttons')) {
     ...
     * @return string
     */
    function evdpl_plugin_fw_get_action_buttons($actions, $echo = true) {
        $actions_html = '';

        foreach ($actions as $action) {
            $action['type'] = 'action-button';

            $actions_html .= evdpl_plugin_fw_get_component($action, $echo);
        }
