...
jQuery( function ( $ ) {
	...
	function dependencies_handler( id, deps, values, type ) {
		...
		var $current_field     = $( id ),
			$current_container = $( id + '-container' ).closest( 'tr' ); // container for EVDPL Plugin Panel

		if ( $current_container.length < 1 ) {
			// container for EVDPL Plugin Panel WooCommerce
			$current_container = $current_field.closest( '.evdpl-plugin-fw-panel-wc-row, .evdpl-toggle-content-row' );
		}
		...
		for ( j in types ) {
			...
			if ( !result ) {
				switch ( current_type ) {
					case 'disable':
						$current_container.addClass( 'evdpl-disabled' );
				...
				switch ( current_type ) {
					case 'disable':
						$current_container.removeClass( 'evdpl-disabled' );
	...
	$( document ).on( 'evdpl-add-box-button-toggle', init_dependencies );
	...
	$( '.evdpl-plugin-fw-panel .woo-nav-tab-wrapper' ).removeClass( 'woo-nav-tab-wrapper' ).addClass( 'evdpl-nav-tab-wrapper' );

	var wrap    = $( '.wrap.evdpl-plugin-ui' ).first(),
	...
	( function () {
		var active_subnav = $( '.evdpl-nav-sub-tab.nav-tab-active' ),
			subnav_wrap   = $( '.evdpl-plugin-fw-wp-page__sub-tab-wrap' );

		if ( active_subnav.length && !subnav_wrap.length ) {
			var mainWrapper = $( '.evdpl-plugin-fw-wp-page-wrapper' );
			if ( !mainWrapper.length ) {
				mainWrapper = $( '#wpbody-content > .evdpl-plugin-ui' );
			}

			if ( mainWrapper ) {
				var defaultWrap = mainWrapper.find( '.evdpl-admin-panel-content-wrap' ); // at first, search for default wrap.
				if ( defaultWrap.length ) {
					defaultWrap.addClass( 'has-subnav' );
				} else {
					// try to wrap a generic wrap div in main wrapper
					mainWrapper.find( '.wrap' ).wrap( '<div class="evdpl-plugin-fw-wp-page__sub-tab-wrap"></div>' );
				}
			}
		}
	} )();
	...
	// Float save button.
	( function () {
		var floatSaveButton = $( '#evdpl-plugin-fw-float-save-button' ),
		...
		if ( floatSaveButton.length > 0 && mainForm.length > 0 ) {
			...
			$( document ).on( 'click', '#evdpl-plugin-fw-float-save-button', function ( e ) {
				e.preventDefault();
				...
				$.post( document.location.href, mainForm.serialize() )
					.done( function ( response ) {
						floatSaveButton.unblock()
							.addClass( 'green' )
							.fadeOut( 300 )
							.html( '<i class="evdpl-icon evdpl-icon-check"></i>' + floatSaveButton.data( 'saved-label' ) )
							.fadeIn( 300 )
							.delay( 2500 )
							.queue(
								function ( next ) {
									floatSaveButton.fadeOut(
										...
										function () {
											...
											$( this ).html( '<i class="evdpl-icon evdpl-icon-save"></i>' + $( this ).data( 'default-label' ) ).fadeIn( 500 );
										}
									);
									next();
								} );
						...
						$( document ).trigger( 'evdpl-plugin-fw-float-save-button-after-saving', [response] );
					} );
			} )
		}
	} )();
