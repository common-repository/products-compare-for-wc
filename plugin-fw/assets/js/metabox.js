...
( function ( $ ) {
	...
	$( actPageOption ).insertAfter( '#evdpl-post-setting .handlediv' );
	$( actPageOption ).insertAfter( '#evdpl-page-setting .handlediv' );

	actPageOptionContainer.on( 'click', function () {
		if ( $( '#_active_page_options' ).is( ":checked" ) ) {
			$( '#evdpl-page-setting .inside .metaboxes-tab, #evdpl-post-setting .inside .metaboxes-tab' ).css( {
																											   'opacity'       : 1,
																											   'pointer-events': 'auto'
																										   } );
		} else {
	...
	$( document.body ).on( 'evdpl-plugin-fw-metabox-init-deps', function () {
		$( document.body ).trigger( 'evdpl-plugin-fw-init-radio' );
		$( '.metaboxes-tab [data-dep-target]:not(.evdpl-plugin-fw-metabox-deps-initialized)' ).each( function () {
			var t = $( this );
			...
			t.addClass( 'evdpl-plugin-fw-metabox-deps-initialized' );
		} );
	} ).trigger( 'evdpl-plugin-fw-metabox-init-deps' );
