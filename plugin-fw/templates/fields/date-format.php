...
list ( $field_id, $class, $name, $js, $format, $value, $data, $custom_attributes ) = evdpl_plugin_fw_extract($field, 'id', 'class', 'name', 'js', 'format', 'value', 'data', 'custom_attributes');

$class = isset($class) ? $class : '';
$class = 'evdpl-plugin-fw-radio evdpl-plugin-fw-date-format ' . $class;

$format = isset($format) ? $format : 'date';
$options = 'time' === $format ? evdpl_get_time_formats() : evdpl_get_date_formats($js);
$custom = true;
...
wp_enqueue_script('evdpl-date-format');
?>
<div class="<?php echo esc_attr($class); ?>"
     ...
     >
         <?php foreach ($options as $key => $label) : ?>
             ...
             ?>
        <div class="evdpl-plugin-fw-radio__row">
            <input type="radio" id="<?php echo esc_attr($radio_id); ?>" name="<?php echo esc_attr($name); ?>"
                   class="evdpl-plugin-fw-date-format__option"
    ...
    <div class="evdpl-plugin-fw-radio__row">
        <input type="radio" id="<?php echo esc_attr($radio_id); ?>" name="<?php echo esc_attr($name); ?>"
               ...
               />
        <label for="<?php echo esc_attr($radio_id); ?>"> <?php esc_html_e('Custom:', 'evdpl-plugin-fw'); ?></label>
        <input type="text" name="<?php echo esc_attr($name . '_text'); ?>"
               id="<?php echo esc_attr($radio_id); ?>_text" value="<?php echo esc_attr($value); ?>"
               class="small-text evdpl-date-format-custom"/>
        <p>
            <strong><?php esc_html_e('Preview:', 'evdpl-plugin-fw'); ?></strong>
