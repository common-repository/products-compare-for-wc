...
list ( $field_id, $class, $name, $selects, $value ) = evdpl_plugin_fw_extract($field, 'id', 'class', 'name', 'selects', 'value');
...
?>
<div class="evdpl-plugin-fw-multi-select" id="<?php echo esc_attr($field_id); ?>">
    <?php for ($i = 0; $i < $selects_count; $i++) : ?>
        <?php if (!( $i % 2 )) : ?>
            <div class="evdpl-select-group">
            <?php endif; ?>

            <div class="evdpl-single-select">
                <?php
                $select = $selects[$i];
                $select['type'] = 'select';
                ...
                <?php evdpl_plugin_fw_get_field($select, true, false); ?>
            </div>
