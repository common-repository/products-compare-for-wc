...
list ( $buttons ) = evdpl_plugin_fw_extract($field, 'buttons');
?>
<?php if (!empty($buttons) && is_array($buttons)) : ?>
    <?php foreach ($buttons as $button) : ?>
        ...
        list ( $button_class, $button_name, $button_data ) = evdpl_plugin_fw_extract($button, 'class', 'name', 'data');
        ?>
        <input type="button" class="<?php echo esc_attr($button_class); ?> button button-secondary"
               value="<?php echo esc_attr($button_name); ?>" <?php echo evdpl_plugin_fw_html_data_to_string($button_data); ?>/>
    <?php endforeach; ?>
<?php endif; ?>
