...
list ( $field_id, $class, $name, $colorpickers, $value ) = evdpl_plugin_fw_extract($field, 'id', 'class', 'name', 'colorpickers', 'value');
...
$class = !empty($class) ? $class : 'evdpl-plugin-fw-multi-colorpicker';
...
<div class="<?php echo esc_attr($class); ?>" id="<?php echo esc_attr($field_id); ?>">
    <?php for ($i = 0; $i < $color_pickers_count; $i++) : ?>
        ...
        ?>
        <div class="evdpl-colorpicker-group">
            <?php foreach ($items_to_process as $color_picker) : ?>
                ...
                ?>
                <div class="evdpl-single-colorpicker colorpicker">
                    <label for="<?php echo esc_attr($color_picker['id']); ?>"><?php echo esc_html($color_picker['title']); ?></label>
                    <?php evdpl_plugin_fw_get_field($color_picker, true, false); ?>
                </div>
            <?php endforeach; ?>
