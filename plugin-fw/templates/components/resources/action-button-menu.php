 ...
 * @package EVDPL\PluginFramework\Templates\Components\Resources
 */
defined('ABSPATH') || exit; // Exit if accessed directly.
?>
<span class="evdpl-plugin-fw__action-button__menu">
    <?php foreach ($action_button_menu as $menu_key => $menu_item) : ?>
        ...
        $item_classes = array('evdpl-plugin-fw__action-button__menu__item', "evdpl-plugin-fw__action-button__menu__item--{$menu_key}-key", $item_class);

        if (isset($item_confirm_data['title'], $item_confirm_data['message']) && !!$item_url) {
            $item_classes[] = 'evdpl-plugin-fw__require-confirmation-link';
        ...
        <?php if (!!$item_url) : ?>
            <a
                ...
                <?php endif; ?>
                <?php
                evdpl_plugin_fw_html_attributes_to_string($item_attributes, true);
                evdpl_plugin_fw_html_data_to_string($item_data, true);
                ?>
